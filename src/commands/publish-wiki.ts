...
import { executeShellCommands, executeTerminalCommands, matchRepoFullName } from "../utils";

export default async function publishWiki(env: Environment) {
  // Check if the current workspace is a valid wiki workspace
  const isWikiWorkspace = env.configuration.get<boolean>("workspace.isWikiWorkspace", false);
  ...
  if (remoteRepoFullName !== repoFullName) {
    vscode.window.showErrorMessage("The git origin does not match the wiki's remote.");
    return;
  }
  ...
  const orphanCommit = (
    await vscode.window.showQuickPick([
      ...
      },
      {
        ...
        detail: "Squash all your changes, including your previous changes, into one commit.",
  ...
  if (!commitMessage) {
    vscode.window.showErrorMessage("Operation cancelled! Please enter a commit message.");
    return;
  }
  ...
  vscode.window.showInformationMessage("Wiki published!", "Open Wiki").then((result) => {
    ...
    vscode.env.openExternal(vscode.Uri.parse(`https://github.com/${repoFullName.replace(".wiki", "")}/wiki`));
  });
}
